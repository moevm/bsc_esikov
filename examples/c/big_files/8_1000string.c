...
//Функции для рамки второго типа=============================================================
void square_proportion_vertical(PNG_FILE* image,int x1,int y1,int x2,int y2,int width,int R,int G,int B){
		choise_color("random",&R,&G,&B);
		koh_proportion_write(image,x1+2*width,y1+2*width,x2-2*width,y1+2*width,width,-1,1,R,G,B);
                koh_proportion_write(image,x1+2*width,y1+2*width,x1+2*width,y2-2*width,width,1,1,R,G,B);
                koh_proportion_write(image,x1+2*width,y2-2*width,x2-2*width,y2-2*width,width,1,1,R,G                                                                                                                                        ,B);
                koh_proportion_write(image,x2-2*width,y1+2*width,x2-2*width,y2-2*width,width,-1,1,R,G,B                                                                                                                                          );

}
//===========================================================================================
int write_frame(PNG_FILE* image,int type,char *color, int width){
		int max;
                int x1,x2;
                int y1,y2;
                int R=0,G=0,B=0;
                choise_color(color,&R,&G,&B);
	if(type==1){//Рамка первого типа
		if(image->height>image->width)
			max=round((double)image->width/(double)122)-1;
		else{
			max=round((double)image->height/(double)122)-1;
		}
		if(width>max){
			printf("Вы вышли за границу возможной ширины рамки 1-го типа(максимальная ширина %d, попробуйте снова\n",max);
			return 0;
		}
		y1=koh_proportion_write(image,2*width,2*width,image->width-2*width,2*width,width,-1,3,R,G,B);
		x1=koh_proportion_write(image,2*width,2*width,2*width,(image->height)-2*width,width,1,3,R,G,B);
		y2=koh_proportion_write(image,2*width,(image->height)-2*width,image->width-2*width,(image->height)-2*width,width,1,3,R,G																	,B);
		x2=koh_proportion_write(image,image->width-2*width,2*width,image->width-2*width,(image->height)-2*width,width,-1,3,R,G,B																	  );
		write_line(image,x1+width,y1+width,x2-width,y1+width,width,R,G,B,0);
		write_line(image,x1+width,y1+width,x1+width,y2-width,width,R,G,B,0);
		write_line(image,x1+width,y2-width,x2-width,y2-width,width,R,G,B,0);
		write_line(image,x2-width,y1+width,x2-width,y2-width,width,R,G,B,0);
		return 1;
	}
	//Второй тип рамки
	else if(type==2){
		if(image->height>image->width){
			max=(int)(image->width*0.02);
		}
		else
			max=(int)(image->height*0.02);
		if(width<0||width>max){
			printf("Превышена возможна ширина рамки второго типа(максимальная ширина %d)\n",max);
			return 0;
		}

		y1=koh_proportion_write(image,2*width,2*width,image->width-2*width,2*width,width,-1,1,R,G,B);
                x1=koh_proportion_write(image,2*width,2*width,2*width,(image->height)-2*width,width,1,1,R,G,B);
                y2=koh_proportion_write(image,2*width,(image->height)-2*width,image->width-2*width,(image->height)-2*width,width,1,1,R,G                                                                                                                                        ,B);
                x2=koh_proportion_write(image,image->width-2*width,2*width,image->width-2*width,(image->height)-2*width,width,-1,1,R,G,B                                                                                                                                          );
		square_proportion_vertical(image,x1,y1,x2,y2,width,R,G,B);
		return 1;
	}
	else{
		printf("Данный тип рамки не существует, возможные(1 и 2)\n");
		return 0;
	}
}
